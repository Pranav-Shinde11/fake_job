...
from jobseakerapp.send_email import sendmail
import random
...
def jobseaker_register(req):
    if req.method == 'POST' and req.FILES["pic"]:
                    ...
                    sendmail(username, email, gen_otp, "Otp Verification")
        ...
        if user_id:
            try:
                ...
                if url:
                    ...
                    title_elem = soup.find('div', attrs={'class': 'heading_4_5 profile'})
                    title = title_elem.get_text() if title_elem else "Title Not Found"

                    company_elem = soup.find('div', attrs={'class': 'heading_6 company_name'})
                    ...
                    openings=0
                    for j in  details.find_all('div',attrs={'class':"text-container"})[:-2]:
                          openings = j
                    openings_final = openings.get_text().strip()
